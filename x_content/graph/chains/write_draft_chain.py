...
3. 章节名称 (`chapter_title`)是撰写内容的主题核心，所有内容需围绕该名称展开。
4. 章节简介 (`chapter_content`)提供写作方向，需以此为基础扩展和深化内容。
5. 所有需要图片说明的部分，提供明确的文字描述，便于后续生成工具操作。每个章节至少配有一幅图，描述其作用、含义及相关内容。
6. 避免技术性模糊语言，确保所有细节通俗易懂且符合专业水准。
7. (`min_length`) 参数 设置文章的最少字数，确保内容的深度与广度，避免过于简略。
...
chapter_content: "初步设想了系统架构框架，需要详细设计与模块化说明。"
min_length: 1200
输出：
