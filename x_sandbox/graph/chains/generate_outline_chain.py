...
#### 主要目标
1. 根据输入的总结（summarization）提取关键信息，构建逻辑清晰的章节和子章节结构。
...
#### 示例输入与输出
**输入**:
- 文档总结（summarization）: “本投标书主要展示公司在邮件系统领域的专业能力及对客户需求的全面理解。”
- 大纲层级（tier）: 2
...
#### 输入
- 文档总结（summarization）:{summarization}
- 大纲层级（tier）: {tier}
"""
...
prompt = PromptTemplate(
    template=template,
    input_variables=["summarization", "tier"],
